...
import { CheckCircle2, Clock3, ImagePlus, Plus, Sparkles, Trophy, Users } from 'lucide-react';
...
type Task = { id: string; target_id: string; scheduled_date: string; assigned_to: string | null; status: 'pending' | 'overdue' | 'completed'; estimated_minutes: number; completed_at: string | null; photo_path: string; points_awarded: number };
...
export default function CleaningRoute() {
  ...
  const load = useCallback(async () => {
    ...
    const [targetRes, taskRes, staffRes] = await Promise.all([
      ...
      client.from('cleaning_tasks').select('id,target_id,scheduled_date,assigned_to,status,estimated_minutes,completed_at,photo_path,points_awarded').eq('brand_id', brandId).order('scheduled_date'),
