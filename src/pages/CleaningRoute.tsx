...
type Task = { id: string; target_id: string; scheduled_date: string; assigned_to: string | null; status: 'pending' | 'overdue' | 'completed'; estimated_minutes: number; started_at: string | null; completed_at: string | null; photo_path: string; points_awarded: number };
...
export default function CleaningRoute() {
  ...
  const load = useCallback(async () => {
    ...
    const [targetRes, taskRes, staffRes] = await Promise.all([
      ...
      client.from('cleaning_tasks').select('id,target_id,scheduled_date,assigned_to,status,estimated_minutes,started_at,completed_at,photo_path,points_awarded').eq('brand_id', brandId).order('scheduled_date'),
    ...
    setTasks(asRows(taskRes.data).map((row) => ({ id: String(row.id), target_id: String(row.target_id), scheduled_date: String(row.scheduled_date), assigned_to: row.assigned_to ? String(row.assigned_to) : null, status: String(row.status) as Task['status'], estimated_minutes: Number(row.estimated_minutes), started_at: row.started_at ? String(row.started_at) : null, completed_at: row.completed_at ? String(row.completed_at) : null, photo_path: String(row.photo_path ?? ''), points_awarded: Number(row.points_awarded ?? 0) })));
    setStaff(asRows(staffRes.data).map((row) => ({ id: String(row.id), display_name: String(row.display_name) })));
    setLoading(false);
  }, [brandId]);
  ...
  const targetById = new Map(targets.map((target) => [target.id, target]));
    const score = targets.length ? Math.round(Math.max(0, 100 - overdue.length * 15 - taskCards.filter((task) => !task.assigned_to).length * 5 + completedToday.length * 5)) : 100;
  const leaders = useMemo(() => staff.map((member) => ({ ...member, points: tasks.filter((task) => task.assigned_to === member.id && task.status === 'completed').reduce((sum, task) => sum + task.points_awarded, 0) })).sort((a, b) => b.points - a.points).slice(0, 3), [staff, tasks]);
  ...
  return <div className="space-y-6"><header className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h1 className="text-3xl font-bold">التنظيفات</h1><p className="text-muted-foreground">نظافة دورية بسيطة، توزيع يدوي، واستلام المدير بالصور.</p></div><Button onClick={() => setTargetDialogOpen(true)}><Plus className="ml-2 h-4 w-4" />إضافة مكان أو معدة</Button></header>
    ...
    <section><div className="mb-3 flex items-center justify-between"><div><h2 className="text-xl font-bold">مهام التنظيف الحالية</h2><p className="text-sm text-muted-foreground">اختر الموظف بجانب كل مكان أو معدة، ثم يستلمها المدير بصورة.</p></div><Badge variant="outline">المطلوب صورة عند الاستلام</Badge></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{taskCards.map((task) => { const target = targetById.get(task.target_id); const assigned = staff.find((member) => member.id === task.assigned_to); return <Card key={task.id} className={task.status === 'overdue' ? 'border-destructive/40' : ''}><CardHeader className="pb-3"><div className="flex justify-between"><div><CardTitle>{target?.icon} {target?.name ?? 'مهمة تنظيف'}</CardTitle><CardDescription>{target?.kind === 'equipment' ? 'معدة' : 'منطقة'} • كل {target?.frequency_days} يوم</CardDescription></div><Badge variant={task.status === 'overdue' ? 'destructive' : 'secondary'}>{task.status === 'overdue' ? 'متأخرة' : 'مستحقة'}</Badge></div></CardHeader><CardContent className="space-y-3"><div className="rounded-lg bg-muted p-3 text-sm"><Clock3 className="ml-1 inline h-4 w-4" /> الوقت المتوقع: {task.estimated_minutes} دقيقة</div><select className="h-10 w-full rounded-md border bg-background px-3" value={task.assigned_to ?? ''} onChange={(event) => void assign(task.id, event.target.value)}><option value="">اختر الموظف يدويًا</option>{staff.map((member) => <option key={member.id} value={member.id}>{member.display_name}</option>)}</select>{assigned && <p className="text-sm text-muted-foreground"><Users className="ml-1 inline h-4 w-4" />مسندة إلى: {assigned.display_name}</p>}{task.started_at ? <div className="rounded-lg bg-emerald-500/10 p-3 text-sm text-emerald-700"><Clock3 className="ml-1 inline h-4 w-4" />بدأ التنفيذ الفعلي</div> : <Button className="w-full" variant="outline" disabled={!task.assigned_to} onClick={() => void start(task.id)}><Play className="ml-2 h-4 w-4" />بدء التنفيذ</Button>}<Button className="w-full" disabled={!task.assigned_to} onClick={() => setCompleteTask(task)}><CheckCircle2 className="ml-2 h-4 w-4" />استلام التنظيف</Button></CardContent></Card>; })}{taskCards.length === 0 && <Card className="md:col-span-2 xl:col-span-3"><CardContent className="p-8 text-center text-muted-foreground">لا توجد مهام معلقة الآن. أضف مكانًا أو معدة وحدد تكرار تنظيفها.</CardContent></Card>}</div></section>
