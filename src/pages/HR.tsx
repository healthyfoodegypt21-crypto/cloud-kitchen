...
import { CalendarClock, Clock3, Plus, Search, UsersRound, WalletCards } from 'lucide-react';
...
type Employee = { id: string; employee_code: string; full_name: string; phone: string; role_title: string; salary: number; is_active: boolean; notes: string; user_id: string | null };
...
type Draft = { full_name: string; employee_code: string; phone: string; role_title: string; salary: string; notes: string };
const initialDraft: Draft = { full_name: '', employee_code: '', phone: '', role_title: '', salary: '', notes: '' };
...
export default function HR() {
  ...
  const load = useCallback(async () => {
    ...
    const [employeeRes, attendanceRes] = await Promise.all([
      (supabase as any).from('employees').select('id,employee_code,full_name,phone,role_title,salary,is_active,notes,user_id').eq('brand_id', brandId).order('full_name'),
  ...
  const saveEmployee = async (event: FormEvent) => {
    ...
    const { error } = await (supabase as any).from('employees').insert({ brand_id: brandId, employee_code: code, full_name: draft.full_name.trim(), phone: draft.phone.trim(), role_title: draft.role_title.trim(), salary: Number(draft.salary || 0), notes: draft.notes.trim() });
    if (error) { toast.error(error.message); return; }
    toast.success('تمت إضافة الموظف بدون إنشاء حساب دخول'); setDialogOpen(false); setDraft(initialDraft); void load();
  };
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><CardTitle>دليل الموظفين</CardTitle><CardDescription>الموظف هنا لا يحتاج بريدًا أو كلمة مرور إلا إذا أردت إنشاء مستخدم نظام منفصل له.</CardDescription></div><div className="relative"><Search className="absolute right-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pr-9" value={search} onChange={event => setSearch(event.target.value)} placeholder="ابحث بالاسم أو الوظيفة" /></div></div></CardHeader><CardContent className="space-y-3">{visibleEmployees.map(employee => { const record = attendance.find(item => item.employee_id === employee.id); return <div key={employee.id} className="flex flex-col gap-3 rounded-xl border p-4 md:flex-row md:items-center md:justify-between"><div><div className="flex flex-wrap items-center gap-2"><strong>{employee.full_name}</strong><Badge variant="secondary">{employee.role_title || 'بدون مسمى'}</Badge>{!employee.user_id && <Badge variant="outline">بدون حساب دخول</Badge>}{record?.status === 'present' && <Badge className="bg-emerald-600">حاضر اليوم</Badge>}</div><p className="mt-1 text-sm text-muted-foreground">{employee.employee_code} • {employee.phone || 'لا يوجد هاتف'} • الراتب: {employee.salary.toLocaleString('ar-EG')} ج.م</p>{record && <p className="mt-1 text-sm text-muted-foreground">حضور: {record.check_in ? new Date(record.check_in).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' }) : '—'} • انصراف: {record.check_out ? new Date(record.check_out).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' }) : '—'}</p>}</div><Button variant="outline" onClick={() => openAttendance(employee)}><Clock3 className="ml-2 h-4 w-4" />حضور وانصراف</Button></div>})}{visibleEmployees.length === 0 && <p className="py-8 text-center text-muted-foreground">لا يوجد موظفون بعد. أضف العمال والطهاة هنا ليظهروا في توزيع التنظيفات.</p>}</CardContent></Card>
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>إضافة موظف</DialogTitle><DialogDescription>هذا السجل مستقل عن مستخدمي النظام؛ لا يتم إعطاء الموظف أي صلاحية دخول تلقائيًا.</DialogDescription></DialogHeader><form className="grid gap-3" onSubmit={saveEmployee}><Field label="اسم الموظف *" value={draft.full_name} onChange={value => setDraft({ ...draft, full_name: value })} /><div className="grid gap-3 sm:grid-cols-2"><Field label="الوظيفة *" value={draft.role_title} onChange={value => setDraft({ ...draft, role_title: value })} placeholder="عامل نظافة، طباخ..." /><Field label="الراتب الشهري" value={draft.salary} onChange={value => setDraft({ ...draft, salary: value })} type="number" /></div><div className="grid gap-3 sm:grid-cols-2"><Field label="كود الموظف" value={draft.employee_code} onChange={value => setDraft({ ...draft, employee_code: value })} placeholder="اختياري" /><Field label="الهاتف" value={draft.phone} onChange={value => setDraft({ ...draft, phone: value })} /></div><div className="space-y-1"><Label>ملاحظات</Label><Textarea value={draft.notes} onChange={event => setDraft({ ...draft, notes: event.target.value })} /></div><Button type="submit">حفظ الموظف</Button></form></DialogContent></Dialog>
