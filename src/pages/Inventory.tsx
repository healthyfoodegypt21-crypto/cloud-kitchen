...
const colors = ['bg-sky-100', 'bg-orange-100', 'bg-violet-100', 'bg-emerald-100', 'bg-rose-100', 'bg-cyan-100'];
...
export default function Inventory() {
  ...
  return <div className="space-y-6">
    ...
    <section><div className="mb-3 flex items-center justify-between"><div><h2 className="text-xl font-bold">الفئات</h2><p className="text-sm text-muted-foreground">اضغط على فئة لعرض كل أصنافها بالتفاصيل.</p></div>{canManage && <div className="flex flex-wrap gap-2"><Button variant="ghost" size="sm" onClick={() => setMode('item')}><PackagePlus className="ml-1 h-4 w-4" />إضافة صنف</Button><Button variant="ghost" size="sm" onClick={() => setMode('category')}><Plus className="ml-1 h-4 w-4" />فئة جديدة</Button>{category !== 'all' && <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive" onClick={() => void deleteCategory()}><Trash2 className="ml-1 h-4 w-4" />حذف الفئة</Button>}</div>}</div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{categoryCards.map((entry, index) => { const value = entry.items.reduce((sum, item) => sum + item.onHand * item.averageCost, 0); const low = entry.items.filter((item) => item.onHand <= item.minStock).length; const active = category === entry.code; return <button key={entry.id} onClick={() => setCategory(entry.code)} className={`group overflow-hidden rounded-2xl border border-slate-100 bg-card p-2 text-right shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${active ? 'ring-2 ring-primary ring-offset-2' : ''}`}><div className={`min-h-56 rounded-xl p-5 ${colors[index % colors.length]}`}><div className="flex items-start justify-between gap-2"><div><h3 className="text-2xl font-bold tracking-tight text-foreground">{entry.name_ar}</h3><p className="mt-2 text-sm leading-6 text-foreground/75">عرض أرصدة الأصناف وقيمتها وحالة إعادة الطلب داخل هذه الفئة.</p></div><Boxes className="h-5 w-5 text-foreground/65" /></div><div className="mt-7 flex flex-wrap gap-2"><span className="rounded-full bg-white/45 px-3 py-1 text-sm font-medium text-foreground/80">{entry.items.length} صنف</span><span className="rounded-full bg-white/45 px-3 py-1 text-sm font-medium text-foreground/80">{formatEGPCurrency(value)}</span>{low > 0 && <span className="rounded-full bg-amber-200/70 px-3 py-1 text-sm font-medium text-amber-950">{low} يحتاج شراء</span>}</div></div><div className="flex items-center justify-between px-3 py-4"><span className="text-lg font-bold">استعراض الأصناف</span><span className="grid h-10 w-10 place-items-center rounded-lg bg-muted transition group-hover:bg-primary group-hover:text-primary-foreground"><ArrowLeft className="h-5 w-5" /></span></div></button>; })}</div></section>
...
function ItemCard({ item, batches, lastMovement, canManage, onDelete, onPurchase, onAction, onCount }: { item: OperationalInventoryItem; batches: InventoryBatch[]; lastMovement?: import('@/types/operationalInventory').OperationalMovement; canManage: boolean; onDelete: () => void; onPurchase: () => void; onAction: (action: MovementAction) => void; onCount: () => void }) {
