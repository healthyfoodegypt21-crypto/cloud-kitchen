...
export default function Inventory() {
  ...
  const startPurchaseRequest = (item: OperationalInventoryItem) => {
    ...
    setLines([{ itemId: item.id, quantity: String(suggestedQuantity), unitCost: String(item.lastPurchasePrice || item.averageCost || 0), locationName: item.locationName, reason: 'إعادة طلب عند الحد الأدنى' }]);
    setMode('purchase');
  };
...
function ItemCard({ item, lastMovement, canManage, onPurchase, onAction, onCount }: { item: OperationalInventoryItem; lastMovement?: import('@/types/operationalInventory').OperationalMovement; canManage: boolean; onPurchase: () => void; onAction: (action: MovementAction) => void; onCount: () => void }) {
