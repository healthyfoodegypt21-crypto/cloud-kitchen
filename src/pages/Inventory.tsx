...
type DraftLine = { itemId: string; quantity: string; unitCost?: string; locationName: string; reason: string };
...
export default function Inventory() {
  ...
  const [line, setLine] = useState<DraftLine>({ itemId: '', quantity: '', unitCost: '', locationName: 'main', reason: '' });
  ...
  const reset = () => { setMode(null); setLine({ itemId: '', quantity: '', unitCost: '', locationName: 'main', reason: '' }); setLines([]); setSupplierName(''); setNotes(''); };
  const addLine = () => {
    if (!line.itemId || number(line.quantity) <= 0) { toast.error('اختر صنفًا وأدخل كمية صحيحة'); return; }
    setLines((current) => [...current, { ...line }]);
    setLine({ itemId: '', quantity: '', unitCost: '', locationName: 'main', reason: '' });
  };
  ...
  const submitPurchase = async () => {
    ...
    const result = await invoke('inventory_submit_purchase_request', { _brand_id: brandId, _supplier_name: supplierName, _lines: lines.map((entry) => ({ item_id: entry.itemId, quantity: number(entry.quantity), unit_cost: number(entry.unitCost || '0'), location_name: entry.locationName, notes: entry.reason })), _notes: notes });
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap gap-2"><select className="h-10 rounded-md border bg-background px-3" value={brandId} onChange={(event) => setBrandId(event.target.value)}>{brands.map((brand: Brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select><Input className="max-w-xs" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="بحث باسم الصنف أو الكود" /></div>
    ...
    <section><div className="mb-3 flex items-center justify-between"><div><h2 className="text-xl font-bold">الفئات</h2><p className="text-sm text-muted-foreground">اضغط فئة لعرض أصنافها.</p></div>{canManage && <Button variant="ghost" size="sm" onClick={() => setMode('category')}><Plus className="ml-1 h-4 w-4" />فئة جديدة</Button>}</div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{categoryCards.map((entry, index) => { const value = entry.items.reduce((sum, item) => sum + item.onHand * item.averageCost, 0); const low = entry.items.filter((item) => item.onHand <= item.minStock).length; return <button key={entry.id} onClick={() => setCategory(entry.code)} className={`rounded-xl border p-4 text-right transition hover:-translate-y-0.5 ${colors[index % colors.length]}`}><div className="flex justify-between"><strong>{entry.name_ar}</strong><Badge>{entry.items.length} صنف</Badge></div><p className="mt-3 text-sm">القيمة: {formatEGPCurrency(value)}</p><p className="text-xs text-muted-foreground">{low ? `${low} أصناف تحتاج إجراء` : 'الرصيد مستقر'}</p></button>; })}</div></section>

    <section className="space-y-3"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">{category === 'all' ? 'كل الأصناف' : categories.find((entry) => entry.code === category)?.name_ar}</h2>{category !== 'all' && <Button variant="ghost" size="sm" onClick={() => setCategory('all')}>عرض الكل</Button>}</div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visibleItems.map((item) => <ItemCard key={item.id} item={item} canManage={canManage} onAction={(action) => { setLine({ itemId: item.id, quantity: '', unitCost: String(item.averageCost), locationName: item.locationName, reason: '' }); setMovementAction(action); setMode('movement'); }} onCount={() => { setLine({ itemId: item.id, quantity: String(item.onHand), unitCost: '', locationName: item.locationName, reason: '' }); setMode('count'); }} />)}</div>{visibleItems.length === 0 && <Card><CardContent className="p-8 text-center text-muted-foreground">لا توجد أصناف في هذه الفئة. أضف صنفًا جديدًا للبدء.</CardContent></Card>}</section>
...
function ItemCard({ item, canManage, onAction, onCount }: { item: OperationalInventoryItem; canManage: boolean; onAction: (action: MovementAction) => void; onCount: () => void }) { const value = item.onHand * item.averageCost; const low = item.onHand <= item.minStock; return <Card className={low ? 'border-amber-300' : ''}><CardHeader className="pb-3"><div className="flex items-start justify-between gap-2"><div><CardTitle className="text-lg">{item.name}</CardTitle><CardDescription>{item.code} • {item.locationName}</CardDescription></div><Badge variant={low ? 'destructive' : 'secondary'}>{low ? 'منخفض' : 'مستقر'}</Badge></div></CardHeader><CardContent className="space-y-3"><div className="grid grid-cols-3 gap-2 text-center"><Metric label="المتاح" value={`${item.onHand} ${item.unit}`} /><Metric label="متوسط الوحدة" value={formatEGPCurrency(item.averageCost)} /><Metric label="قيمة الصنف" value={formatEGPCurrency(value)} /></div>{canManage && <div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" onClick={() => onAction('deposit')}>إيداع</Button><Button size="sm" variant="outline" onClick={() => onAction('withdraw')}>سحب</Button><Button size="sm" variant="outline" className="text-destructive" onClick={() => onAction('waste')}><Trash2 className="ml-1 h-3.5 w-3.5" />هالك</Button><Button size="sm" variant="outline" onClick={onCount}>جرد</Button></div>}</CardContent></Card>; }
